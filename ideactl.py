...
def is_subrepo(fname):
    return opts.subrepo in fname
...
def dwim(opts):
    ...
    opts.subrepo = 'com-liferay-dynamic-data-mapping'
    opts.src = src_root
...
if __name__ == "__main__":
    ...
    with open(opts.prj or os.devnull, mode='w') as pf:
        ...
        for fn in walk(os.path.dirname(src), keep=is_iml, recur=negate(is_ignorable)):
            if opts.iml:
                n = fix_iml(fn)
